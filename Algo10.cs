...
namespace AlgorithmsDataStructures
{
    ...
    public class PowerSet<T>
    {
        ...
        public int SeekSlot(T value)
        {
            ...
            if (EqualityComparer<T>.Default.Equals(slots[index], default(T)))
            {
                return index;
            }
            else
            {
                if (EqualityComparer<T>.Default.Equals(slots[index], value))
                {
                    return -1;
                }
                index = index + step;
                ...
                while (index != indexHash)
                {
                    ...
                    else if (EqualityComparer<T>.Default.Equals(slots[index], value))
                    {
                        return -1;
                    }
                    if ((index + step) >= size)
        ...
        public List<T> DeleteEmptySlots(T [] array)
        {
            ...
            foreach (T item in array)
            {
                ...
                if (!EqualityComparer<T>.Default.Equals(item, default(T))) list.Add(item);
            }
            return list;
        }
    }
